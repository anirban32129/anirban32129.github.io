...
=====================================================
THIRD BUG FIX (v7) — MODE-SWITCH TIMING BUG
=====================================================
Every mode_*.html file ends with a small script that force-selects
its own mode on load, e.g.:

  sel.value = 'algebraproof';
  sel.dispatchEvent(new Event('change'));

This script was a plain <script> tag, but the main engine (which
attaches the 'change' listener that actually switches modes) lives
in a <script type="module"> earlier in the file. Module scripts are
deferred by the browser and run AFTER the document finishes parsing;
a later plain <script> runs immediately, before that. So the mode-set
script was firing before any listener existed to react to it —
the dropdown's value got set, but nothing responded, and the page
fell back to whatever mode is first in the list (Addition).

This is why mode_add.html looked fine (its target mode already
matches the default) while every other mode_*.html file silently
loaded the Addition scene instead of its own.

Fixed by changing that trailing script to <script type="module">
in all 47 affected files, so it now runs in its correct order —
after the engine's listener is attached, not before.
